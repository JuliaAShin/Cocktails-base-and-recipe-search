...
function getRecipe() {
    ...
    if (userCocktailName.length == 0) {
        recipe.innerHTML = `<p>Please type a name of cocktail</p>`
    } else {
        fetch(url1 + userCocktailName)
            ...
            .catch(() => {
                recipe.innerHTML = `<p>Please type correct name of cocktail</p>`;
            });
    }
};
...
function getListByFirstLetter() {
    ...
    if (userLetter.length == 0) {
        listByFirstBtn.innerHTML = `<p>Please type any first letter</p>`
    } else {
